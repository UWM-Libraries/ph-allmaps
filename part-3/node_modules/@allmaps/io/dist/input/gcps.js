 ...
 */
export function parseGcps(gcpString, options) {
    ...
    const gcpFileFormat = options?.gcpFileFormat ?? parseGcpFileFormatFromGcpString(gcpLines);
    const gcpProjection = options?.gcpProjection ??
    ...
    const defaultParseGcpsOptions = {
        ...
        gcpResourceYAxis: gcpFileFormat &&
            supportedGcpFileFormatsWithResourceYAxisUp.includes(gcpFileFormat)
            ? 'up'
            : 'down'
    };
    const mergedOptions = mergeOptionsUnlessUndefined(defaultParseGcpsOptions, options);
    // Parse
    let gcps = parseGcpLines(gcpLines, mergedOptions);
    // Check for invalied coordinates
    const allGcpsValid = gcps.every(({ geo, resource }) => geo.every((c) => !isNaN(c)) && resource.every((c) => !isNaN(c)));
    if (!allGcpsValid) {
    ...
    gcps = gcps.map((gcp) => {
        ...
        if (mergedOptions.gcpResourceWidth || mergedOptions.gcpResourceHeight) {
            if (!mergedOptions.resourceWidth || !mergedOptions.resourceHeight) {
                throw new Error('Resource width and height required when parsing GCPs with gcp resource width and height');
            }
            if (!mergedOptions.gcpResourceWidth || !mergedOptions.gcpResourceHeight) {
                throw new Error('GCP resource width and height required when parsing GCPs with gcp resource width and height');
            }
            gcp = {
                resource: [
        ...
        if (mergedOptions.gcpResourceOrigin === 'bottom-left') {
            if (!mergedOptions.resourceHeight) {
                throw new Error('Resource height required when parsing GCPs with resource origin in bottom left');
            }
            gcp = {
                resource: [
                    gcp.resource[0],
        ...
        if (!isEqualProjection(mergedOptions.gcpProjection, lonLatProjection)) {
            gcp = {
                ...
                geo: proj4(mergedOptions.gcpProjection.definition, lonLatProjection.definition, gcp.geo)
            };
        }
        return gcp;
    });
...
export function parseGcpLines(lines, options) {
...
export function parseQgisGcpLines(lines) {
    const gcps = lines
        .filter((line) => line.slice(0, 4) !== '#CRS' && line.slice(0, 4) !== 'mapX')
        .map((line) => line
        .replace(/\s+/g, '')
        .split(',')
        .map((_c, i, l) => {
...
export function parseArcGisCsvGcpLines(lines) {
    const coordinates = lines
        .map((line) => line.replace(/\s+/g, '').split(',').slice(1).map(Number))
        .map((coordinateArray) => coordinateArrayToGcp(coordinateArray));
    return coordinates;
}
export function parseArcGisTsvGcpLines(lines) {
    const coordinates = lines
        .map((line) => line.split('\t').map(Number))
        .map((coordinateArray) => coordinateArrayToGcp(coordinateArray));
    return coordinates;
}
export function parseGdalGcpLines(lines) {
    return parseGdalCoordinateLines(lines).map((coordinateArray) => coordinateArrayToGcp(coordinateArray, 4));
}
export function parseGdalCoordinateLines(lines) {
    // String from multiline file where each line contains multiple coordinates separated by whitespace
    return lines.map((line) => line.split(/\s+/).map((coordinate) => Number(coordinate.trim())));
}
export function parseGcpProjectionFromGcpString(gcpString) {
    const gcpProjectionDefinition = parseGcpProjectionDefinitionFromGcpString(gcpString);
    let gcpProjection = undefined;
...
export function parseGcpProjectionDefinitionFromGcpString(gcpString) {
...
export function parseGcpProjectionDefinitionFromLine(line) {
...
export function parseGcpFileFormatFromGcpString(lines) {
    if (lines.find((line) => line.slice(0, 4) === 'mapX') != undefined) {
        return 'qgis';
    }
...
function coordinateArrayToGcp(coordinateArray, expectedLength) {
    if (expectedLength && coordinateArray.length !== expectedLength) {
        throw new Error(`Encountered ${coordinateArray.length} numbers instead of ${expectedLength} while parsing GCP from line.`);
    }
    if (coordinateArray.length < 4) {
        throw new Error(`Encountered ${coordinateArray.length} numbers instead of at least 4 while parsing GCP from line.`);
    }
    return {

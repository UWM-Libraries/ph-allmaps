import { lonLatProjection, ProjectedGcpTransformer, projectionToAntialiasedProjection } from '@allmaps/project';
import { generateChecksum, generateId } from '@allmaps/id';
import { bboxToRectangle, bboxToSize, computeBbox, geometryToGeojsonGeometry, mergeOptions, mergeOptionsUnlessUndefined, rectanglesToScale } from '@allmaps/stdlib';
...
export async function generateGeoreferencedMapsGeotiffScripts(maps, options) {
    if (!options) {
        options = {};
    }
    const basenames = [];
    const gdalwarpScripts = [];
    for (const [index, map] of maps.entries()) {
        const projectedTransformer = options.projectedTransformers
            ? options.projectedTransformers[index]
            : undefined;
        const { basename, checkScript, gdalWarpScript } = await generateGeoreferencedMapGdalwarpScripts(map, mergeOptionsUnlessUndefined(options, { projectedTransformer }));
...
export function generateGdalPreamble(options) {
...
export async function generateGeoreferencedMapGdalwarpScripts(map, options) {
    ...
    let imageFilename;
    if (map.resource.id in mergedOptions.imageFilenames &&
    ...
    const geoMask = mergedOptions.projectedTransformer.transformToGeo(map.resourceMask, {
    ...
    const resourceFullMaskSize = [map.resource.width, map.resource.height];
    const resourceMaskBbox = computeBbox(map.resourceMask);
    const resourceMaskRectangle = bboxToRectangle(resourceMaskBbox);
    const projectedGeoMaskRectangle = mergedOptions.projectedTransformer.transformToGeo(resourceMaskRectangle);
    const resourceToProjectedGeoScale = rectanglesToScale(resourceMaskRectangle, projectedGeoMaskRectangle);
    const projectedGeoMask = mergedOptions.projectedTransformer.transformToGeo(map.resourceMask);
    const projectedGeoMaskBboxSize = bboxToSize(computeBbox(projectedGeoMask));
    const size = [
    ...
    return {
        basename,
        checkScript: generateCheckImageExistsAndCorrectSize(imageFilename, basename, resourceFullMaskSize),
        gdalWarpScript: generateGdalwarpScriptInternal(imageFilename, basename, mergedOptions.outputDir, mergedOptions.projectedTransformer.interalProjectedGcps, mergedOptions.projectedTransformer.type, String(projectionToAntialiasedProjection(mergedOptions.projectedTransformer.internalProjection, { type: 'epsg' }).definition), String(projectionToAntialiasedProjection(mergedOptions.projectedTransformer.projection, { type: 'epsg' }).definition), mergedOptions.jpgQuality, geojsonMaskPolygon, size)
    };
}
export function generateGdalbuildvrtScript(basenames, options) {
    const mergedOptions = mergeOptions({ outputDir: '.' }, options);
...
function generateCheckImageExistsAndCorrectSize(imageFilename, basename, resourceSize) {
    return `
...
function generateGdalwarpScriptInternal(imageFilename, basename, outputDir, internalProjectedGcps, transformationType, internalProjectionDefinition, projectionDefinition, jpgQuality, geojsonMaskPolygon, size) {
    ...
    const script = `${transformationMessage ? `echo "${transformationMessage}"` : ''}

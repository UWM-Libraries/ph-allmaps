 ...
 */
export function generateGeoreferencedMapGcps(map, options) {
 ...
 * and will be infered from the map by default.
 */
export function generateGcps(gcps, options) {
    ...
    const defaultParseGcpsOptions = {
        ...
        gcpResourceYAxis: options?.gcpFileFormat &&
            supportedGcpFileFormatsWithResourceYAxisUp.includes(options?.gcpFileFormat)
            ? 'up'
            : 'down'
    };
    ...
    gcps = gcps.map((gcp) => {
        ...
        if (!isEqualProjection(mergedOptions.gcpProjection, lonLatProjection)) {
            gcp = {
                ...
                geo: proj4(lonLatProjection.definition, mergedOptions.gcpProjection.definition, gcp.geo)
            };
        }
        // Process resource origin
        if (mergedOptions.gcpResourceOrigin === 'bottom-left') {
            if (!mergedOptions.resourceHeight) {
                throw new Error('Resource height required when printing GCPs with resource origin in bottom left');
            }
            gcp = {
                resource: [
                    gcp.resource[0],
        ...
        if (mergedOptions.gcpResourceWidth || mergedOptions.gcpResourceHeight) {
            if (!mergedOptions.resourceWidth || !mergedOptions.resourceHeight) {
                throw new Error('Resource width and height required when printing GCPs with gcp resource width and height');
            }
            if (!mergedOptions.gcpResourceWidth || !mergedOptions.gcpResourceHeight) {
                throw new Error('GCP resource width and height required when printing GCPs with gcp resource width and height');
            }
            gcp = {
                resource: [
...
export function generateQgisHeader(options) {
    const headerLines = [];
...
export function generateQgisGcpLines(gcp) {
    const gcpLines = gcp.map((gcp) => [gcp.geo[0], gcp.geo[1], gcp.resource[0], gcp.resource[1]]
        .map(String)
        .join(','));
    return gcpLines;
}
export function generateArcGisCsvGcpLines(gcp) {
    const gcpLines = gcp.map((gcp, index) => [index + 1, gcp.resource[0], gcp.resource[1], gcp.geo[0], gcp.geo[1]]
        .map(String)
        .join(','));
    return gcpLines;
}
export function generateArcGisTsvGcpLines(gcp) {
    const gcpLines = gcp.map((gcp) => [gcp.resource[0], gcp.resource[1], gcp.geo[0], gcp.geo[1]]
        .map(String)
        .join('\t'));
    return gcpLines;
}
export function generateGdalGcpLines(gcp) {
    const gcpLines = gcp.map((gcp) => [gcp.resource[0], gcp.resource[1], gcp.geo[0], gcp.geo[1]]
        .map(String)
        .join(' '));
    return gcpLines;

...
export function getPropertyFromQuadrupleCacheOrComputation(cache, key0, key1, key2, key3, computation, checkUse = () => true, checkStore = () => true) {
    if (cache.get(key0)?.get(key1)?.get(key2)?.has(key3) &&
        checkUse(cache.get(key0)?.get(key1)?.get(key2)?.get(key3))) {
        return cache.get(key0)?.get(key1)?.get(key2)?.get(key3);
    }
    else {
        const result = computation();
        if (checkStore(result)) {

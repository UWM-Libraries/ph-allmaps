...
export function dezoomify() {
    return new Command('dezoomify')
        ...
        .description('Generates a Bash script that runs GDAL to create Cloud Optimized GeoTIFFs from one of more Georeference Annotations')
        .option('-d, --output-dir <dir>', 'Output directory', '.')
        .action(async (imageIds, options) => {
        const outputDir = options.outputDir;
        imageIds = await readLines(imageIds);
        const dezoomifyScripts = [];
        let index = 0;
        for (const imageId of imageIds) {
            const outputFilename = path.join(outputDir, `${await generateId(imageId)}.jpg`);
            const dezoomifyMessage = `echo "Downloading ${imageId} (${index + 1}/${imageIds.length})"`;
        ...
        printString([preamble(options.outputDir), '', ...dezoomifyScripts].join('\n'));
    });
}

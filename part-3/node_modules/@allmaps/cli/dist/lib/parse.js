...
import { transformationTypeToTypeAndOrder, typeAndOrderToTransformationType } from '@allmaps/transform';
import { isPoint, isRing, mergeOptions, mergeOptionsUnlessUndefined, stringToSvgGeometriesGenerator, svgGeometryToGeometry } from '@allmaps/stdlib';
import { parseGcps, parseGcpProjectionFromGcpString, parseGdalCoordinateLines, supportedGcpFileFormats, supportedGcpResourceYAxis, supportedGcpResourceOrigin } from '@allmaps/io';
import { readFromFile, parseJsonFromFile } from './io.js';
export const mustContainOneMapMessage = 'Annotation must contain exactly 1 georeferenced map';
export const mustContainGcpsMessage = 'No GCPs supplied. Supply a Georeference Annotation or a file containing GCPs.';
export const mustContainTransformationTypeMessage = 'No transformation type supplied.';
export function parseMap(annotation) {
    if (annotation) {
...
export function parseAnnotationsValidateMaps(jsonValues, annotationInputs) {
    const maps = jsonValues
        .map((jsonValue) => parseAnnotationValidateMap(jsonValue, annotationInputs))
        .flat();
    return maps;
}
export function parseAnnotationValidateMap(jsonValue, annotationInputs) {
    let mapOrMaps;
    if (jsonValue &&
...
export function mergeAnnotationInputsInMapOrMaps(mapOrMaps, annotationInputs) {
    if (Array.isArray(mapOrMaps)) {
        return mapOrMaps.map((map) => mergeAnnotationInputsInMap(map, annotationInputs));
    }
    else {
        return mergeAnnotationInputsInMap(mapOrMaps, annotationInputs);
    }
}
export function mergeAnnotationInputsInMap(map, annotationInputs) {
    ...
    const transformationTypeAndOrder = mergeOptionsUnlessUndefined(map.transformation ?? { type: undefined }, transformationTypeToTypeAndOrder(annotationInputs?.transformationType));
    ...
    map = mergeOptionsUnlessUndefined(map, {
        ...
        resourceCrs: annotationInputs?.internalProjection ?? annotationInputs?.gcpProjection
    });
    return map;
}
export function parseAnnotationInputOptions(options) {
    const { resourceId, resourceType, resourceWidth, resourceHeight, resourceMask: resourceMaskString } = options;
    ...
    const { gcps, gcpProjection, transformationType, internalProjection, projection } = parseProjectedGcpTransformerInputOptions(options);
    return {
        resourceId,
        resourceType,
        resourceWidth,
...
export function parseProjectedGcpTransformerInputOptions(options) {
    let map;
    try {
...
export function parseProjectedGcpTransformerInputOptionsAndMap(options, map) {
    const { gcps, gcpProjection } = parseGcpInputOptions(options, map);
    const { transformationType } = parseTransformationTypeInputOptions(options, map);
    const { internalProjection } = parseInternalProjectionInputOptions(options, map);
...
export function parseGcpInputOptions(options, map) {
    let gcps;
...
export function parseTransformationTypeInputOptions(options, map) {
...
export function parseInternalProjectionInputOptions(options, map) {
    const internalProjectionInputs = {};
    if (options &&
...
export function parseProjectionInputOptions(options) {
...
export function parseGcpProjectionInputOptions(options) {
    const gcpProjectionInputs = {};
    if (options &&
        typeof options === 'object' &&
...
export function parseProjectedGcpTransformOptions(options) {
    ...
    if (options && typeof options === 'object') {
        if ('maxDepth' in options && options.maxDepth) {
            partialProjectedGcpTransformOptions.maxDepth = Math.round(Number(options.maxDepth));
        }
        if ('minOffsetRatio' in options && options.minOffsetRatio) {
            partialProjectedGcpTransformOptions.minOffsetRatio = Number(options.minOffsetRatio);
        }
        if ('minOffsetDistance' in options && options.minOffsetDistance) {
            partialProjectedGcpTransformOptions.minOffsetDistance = Number(options.minOffsetDistance);
        }
        if ('minLineDistance' in options && options.minLineDistance) {
            partialProjectedGcpTransformOptions.minLineDistance = Number(options.minLineDistance);
        }
...
export function parseRcps(options) {
    if (options.rcps) {
        const rcps = parseJsonFromFile(options.rcps);
        if (Array.isArray(rcps) &&
            rcps.every((rcp) => typeof rcp === 'object' &&
                'type' in rcp &&
                rcp.type === 'rcp' &&
                'id' in rcp &&
...
export function parseAttachInputs(options) {
    ...
    if (options && typeof options === 'object') {
        ...
        if ('useMapTransformationTypes' in options &&
        ...
        if ('noEvaluateAttachmentScps' in options &&
...
export function parseCommanderGcpFileFormatOption(string) {
    if (supportedGcpFileFormats.includes(string)) {
        return string;
    }
    else {
        throw new Error(`Unsupported GCP file format. Supported formats are ${supportedGcpFileFormats.join(', ')}`);
    }
}
export function parseCommanderGcpResourceOrigin(string) {
    if (supportedGcpResourceOrigin.includes(string)) {
        return string;
    }
    else {
        throw new Error(`Unsupported GCP resource origin. Supported formats are ${supportedGcpResourceOrigin.join(', ')}`);
    }
}
export function parseCommanderGcpResourceYAxis(string) {
    if (supportedGcpResourceYAxis.includes(string)) {
        return string;
    }
    else {
        throw new Error(`Unsupported GCP resource y axis. Supported formats are ${supportedGcpResourceYAxis.join(', ')}`);
    }
}

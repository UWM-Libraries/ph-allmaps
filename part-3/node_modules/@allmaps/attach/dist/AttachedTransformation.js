...
import { arrayMatrixSize, groupBy, mergeOptions, multiplyArrayMatrix, newArrayMatrix, pasteArrayMatrix, sliceArrayMatrix } from '@allmaps/stdlib';
import { BaseIndependentLinearWeightsTransformation, solveIndependentlyPseudoInverse } from '@allmaps/transform';
import { ProjectedGcpTransformer } from '@allmaps/project';
import { resourceToSource, sourceDestinationToResourceGeo } from './helper-functions.js';
const defaultAttachedTransformationFromGeoreferencedMapOptions = {
...
const defaultAttachedTransformationOptions = {
...
export class AttachedTransformation {
    ...
    trailingCumulativeCoefsArrayMatrixSizeById;
    coefsArrayMatrices;
    coefsArrayMatrix;
     ...
     */ constructor(transformationsById, attachments, options) {
        ...
        this.ScpsById = Object.values(groupBy(this.attachments.flat(), (Scp) => Scp.id)).map((scpsForId) => scpsForId.filter((scp, index) => scpsForId.findIndex((s) => s.transformationId === scp.transformationId) === index));
    ...
    getDestinationPointsArrays() {
        ...
        this.transformationsById.forEach((transformation) => {
            const transformationDestinationPointsArrays = transformation.getDestinationPointsArrays();
            destinationPointsArrays[0].push(...transformationDestinationPointsArrays[0]);
            destinationPointsArrays[1].push(...transformationDestinationPointsArrays[1]);
        });
    ...
    getCoefsArrayMatrix() {
        ...
        let trailingCumulativeCoefsArrayMatrixSize = [0, 0];
        for (const [transformationId, transformation] of this.transformationsById.entries()) {
            coefsArrayMatrix = pasteArrayMatrix(coefsArrayMatrix, ...trailingCumulativeCoefsArrayMatrixSize, transformation.coefsArrayMatrix);
            this.trailingCumulativeCoefsArrayMatrixSizeById.set(transformationId, trailingCumulativeCoefsArrayMatrixSize);
        ...
        for (const attachment of this.attachments) {
            const transformation0 = this.transformationsById.get(attachment[0].transformationId);
            const transformation1 = this.transformationsById.get(attachment[1].transformationId);
            if (!transformation0) {
                throw new Error(`Transformation not found for transformationId ${attachment[0].transformationId}.`);
            }
            if (!transformation1) {
                throw new Error(`Transformation not found for transformationId ${attachment[1].transformationId}.`);
            }
            const attachementSourcePointCoefsArray0 = transformation0?.getSourcePointCoefsArray(attachment[0].source);
            const attachementSourcePointCoefsArray1 = transformation1?.getSourcePointCoefsArray(attachment[1].source);
            const trailingCumulativeCoefsArrayMatrixSize0 = this.trailingCumulativeCoefsArrayMatrixSizeById.get(attachment[0].transformationId);
            const trailingCumulativeCoefsArrayMatrixSize1 = this.trailingCumulativeCoefsArrayMatrixSizeById.get(attachment[1].transformationId);
            if (!trailingCumulativeCoefsArrayMatrixSize0) {
                throw new Error(`trailingCumulativeCoefsArrayMatrixSize not found for transformationId ${attachment[0].transformationId}.`);
            }
            if (!trailingCumulativeCoefsArrayMatrixSize1) {
                throw new Error(`trailingCumulativeCoefsArrayMatrixSize not found for transformationId ${attachment[1].transformationId}.`);
            }
            coefsArrayMatrix = pasteArrayMatrix(coefsArrayMatrix, trailingCumulativeCoefsArrayMatrixSize[0], trailingCumulativeCoefsArrayMatrixSize0[1], [attachementSourcePointCoefsArray0]);
            coefsArrayMatrix = pasteArrayMatrix(coefsArrayMatrix, trailingCumulativeCoefsArrayMatrixSize[0], trailingCumulativeCoefsArrayMatrixSize1[1], multiplyArrayMatrix([attachementSourcePointCoefsArray1], -1));
            trailingCumulativeCoefsArrayMatrixSize = [
    ...
    processWeightsArrays() {
        ...
        Array.from(this.transformationsById.entries()).forEach(([transformationId, transformation]) => {
            const trailingCumulativeCoefsArrayMatrixSize = this.trailingCumulativeCoefsArrayMatrixSizeById.get(transformationId);
            if (!trailingCumulativeCoefsArrayMatrixSize) {
                throw new Error(`trailingCumulativeCoefsArrayMatrixSize not found for transformationId ${transformationId}.`);
            }
            const transformationIds = Array.from(this.transformationsById.keys());
            const nextId = transformationIds[transformationIds.indexOf(transformationId) + 1];
            const nextTrailingCumulativeCoefsArrayMatrixSize = this.trailingCumulativeCoefsArrayMatrixSizeById.get(nextId);
    ...
    toGeoreferencedMaps() {
        ...
        const projectedGcpTransformersById = this.options?.projectedGcpTransformersById;
        ...
        if (!georeferencedMapsById || !projectedGcpTransformersById) {
            throw new Error('No Georeferenced Maps or GCP Transformers found. Create an Attached Transfromation from GeoreferencedMaps to be able to recreate those.');
        }
        // Gather all points to evaluate: sourceControlPoints and extra sourcePoints
        const spForEvaluation = [];
        if (gcpSourcePoints) {
        ...
        spForEvaluation.forEach((sourcePoint) => {
            sourcePoint.destination = this.evaluateFunction(sourcePoint.source, sourcePoint.transformationId);
        });
        ...
        spForEvaluation.forEach((evaluationPoint) => {
            const projectedGcpTransformer = projectedGcpTransformersById.get(evaluationPoint.transformationId);
            if (!projectedGcpTransformer) {
                throw new Error(`projectedGcpTransformer not found for transformationId ${evaluationPoint.transformationId}.`);
            }
            const transformerOptions = projectedGcpTransformer.getTransformerOptions();
            const { resource, geo } = sourceDestinationToResourceGeo(evaluationPoint, transformerOptions, projectedGcpTransformer);
            Array.from(georeferencedMapsById.entries()).forEach(([mapId, georeferencedMap]) => {
     ...
     */
    static fromGeoreferencedMaps(georeferencedMaps, rcps, options) {
        options = mergeOptions(defaultAttachedTransformationFromGeoreferencedMapOptions, options);
        if (options?.useMapTransformationTypes) {
            delete options.transformationType;
        }
        const georeferencedMapsById = new Map();
        ...
        for (const georeferencedMap of georeferencedMaps) {
            ...
            const projectedGcpTransformer = ProjectedGcpTransformer.fromGeoreferencedMap(georeferencedMap, options);
            ...
            if (!(transformation instanceof BaseIndependentLinearWeightsTransformation)) {
                throw new Error(`Transformation type ${transformation.type} unsupported.`);
            }
        ...
        const attachments = scpsById
            .map((scpsForId) => scpsForId.reduce((attachmentsForId, scpForId, index) => {
            if (index >= 1) {
